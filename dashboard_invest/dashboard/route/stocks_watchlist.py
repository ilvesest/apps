...
from dashboard.logic.io import ioCacheAndLog, styleDf, comment_button
...
def stockswatchlistScript(df:pd.DataFrame) -> dict:
    ...
    # set header including row number to columns.name
    df_watch.columns = df_watch.iloc[0]
    ...
    df_watch_styled = styleDf(df_watch, 'stocks_watchlist')

    df_style = df_watch_styled.hide(axis='index').set_table_attributes('class="stockwatch"')
